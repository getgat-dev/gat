...
[gat ls-files](/commands/ls-files) using `--selection NAME`; matches can change as the lock changes.
...
Save a complete definition with at least one of `--path`, `--include`, or `--exclude`. Use `--path .` for all tracked paths. Adding a selection never chooses a default.

### Usage

```text
Usage: gat selection add [OPTIONS] <--path <PATH>|--include <INCLUDE>|--exclude <EXCLUDE>> <NAME>
```
...
Use --path . to save an unrestricted selection (all tracked paths).
```
...
<Tabs sync={false}>
<Tab title="Save model selection">
...
<Tab title="Save an unrestricted selection">
...
```text
Usage: gat selection update [OPTIONS] <--path <PATH>|--include <INCLUDE>|--exclude <EXCLUDE>|--clear-include|--clear-exclude> <NAME>
```
