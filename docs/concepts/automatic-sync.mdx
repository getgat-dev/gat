...
Sync checks <Tooltip tip="Filesystem information used to detect possible changes without reading the whole file. It is distinct from tracking metadata in gat.lock, which records paths and content IDs.">file metadata</Tooltip> first and hashes content when needed. [gat status](/commands/status)
