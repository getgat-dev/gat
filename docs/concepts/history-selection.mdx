...
these flags with [gat push](/commands/push), [gat fetch](/commands/fetch),
[gat pull](/commands/pull), [gat status --remote](/commands/status), or [gat gc](/commands/gc).
...
For transfers and remote status, combine history with
[path selection](/concepts/path-selection) to choose files within those snapshots.
Garbage collection does not accept path filters or use the default named selection;
it protects every object referenced by its selected locks. Before deleting stored
content, review [garbage-collection protection rules](/commands/gc).
