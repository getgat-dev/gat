...
bookkeeping stay out of Git. Gat owns `.gat/.gitignore`, which ignores the
directory's contents before local storage files are written. A new command that
writes local storage recreates this file if it is missing; exclude sync is only
needed for managed working files. Opening an existing local database also ensures
this protection, since SQLite can create coordination files even for read-only
queries. Reading an absent database leaves local storage absent.
