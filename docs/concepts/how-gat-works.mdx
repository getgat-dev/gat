...
Commit `gat.lock` and project `gat.yaml`. The cache, managed working files, and
local bookkeeping stay out of Git.

<Accordion title="How Gat keeps local storage out of Git">
  Gat maintains `.gat/.gitignore` before writing local storage, recreating it
  if missing. Opening an existing state database also ensures this protection:
  database reads can create coordination files. Reading an absent database
  leaves local storage absent.

  Managed working files use separate Git exclusions. See
  [ignore rules](/guides/improving-performance#combine-file-exclusions) to
  understand which files Git and Gat each ignore.
</Accordion>
...
<AccordionGroup>
  <Accordion title="gat.lock: which content belongs at each path">
    ...
    Large repositories can [shard the lock](/guides/improving-performance#shard-a-very-large-gat-lock)
    into a `gat.lock/` directory. The [logical format](/spec/lock-v1) stays the same.
  </Accordion>
  <Accordion title="gat.yaml: shared settings">
    Project configuration contains general settings and named resources.
    Use `gat config` for settings such as automatic fetching, and `gat remote`,
    `gat route`, `gat mount`, or `gat selection` to manage named definitions.
    Personal overrides belong in global or local configuration.
    See [General settings and managed resources](/concepts/config-inheritance#general-settings-and-managed-resources)
    for the command distinction and scope rules.
  </Accordion>
  ...
  <Accordion title=".gat/state: local bookkeeping">
    ...
    For maintenance and recovery, see the experimental
    [gat system](/commands/system) commands.
  </Accordion>
...
with [gat mv](/commands/mv) reuses its cached content. Editing a file requires `gat add`
...
Press **Ctrl+C** once to request cancellation. Gat stops admitting new transfer
work, interrupts Git fetch/checkout at its checkpoints, and waits for owned work
and cleanup to finish. A filesystem publication or journaled mount update that
has already started finishes its consistency-critical sequence before returning.
Cancellation is cooperative: work without an interruption checkpoint may need to
finish or reach its I/O timeout.

A cancelled CLI invocation exits with status **130**, without a success summary.
On Unix, **SIGTERM** requests the same cleanup and exits with **143**. On Windows,
Ctrl+C uses the same cooperative behavior. Hooks suppress the interruption notice
and keep reporting other errors.

A second interrupt terminates the process immediately and skips remaining
cleanup. Use it only when you intend to force termination; the next invocation
may need to recover an interrupted transaction.
