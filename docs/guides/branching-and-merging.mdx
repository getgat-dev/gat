...
Use Git for branches and commits. Run [gat add](/commands/add) after editing large files,
then publish their bytes with [gat push](/commands/push) before publishing commits.
...
<Tabs sync={false}>
  ...
  <Tab title="Review">
    ...
    [gat diff](/commands/diff) compares tracked paths and content IDs. It does not compare the contents
...
When Git invokes the installed semantic driver for a changed lock file or shard,
Gat merges by decoded path and emits strictly path-ordered rows. Fast-forwards
and one-sided file changes can bypass the driver.

Git's default text merge does not validate lock ordering. Do not use `merge=union`:
it can concatenate conflicting additions out of order.
...
The combined lock must also satisfy file/directory-prefix constraints: for example,
it cannot track both `data` and `data/model.bin`. The driver rejects these conflicts
within each merged file before writing output. It does not certify conflicts across
shards or files Git resolves without invoking the driver; subsequent Gat reads
...
When the merge driver cannot resolve a path, Git records `gat.lock` as unresolved.
Gat leaves the current lock content untouched instead of writing
<Tooltip tip="The lines beginning with <<<<<<<, =======, and >>>>>>> that Git normally inserts into conflicted text files. Gat reports the conflicting paths without inserting these lines into the lock.">conflict markers</Tooltip>.
...
<Steps>
  ...
  <Step title="Produce the final semantic lock">
    ...
    64 lowercase hexadecimal digest characters, a tab, and an unquoted escaped path.
    Encode path controls as lowercase `\xhh`; quotes and spaces are literal.
    Keep rows strictly sorted by decoded path bytes, and terminate the header and
    every row with LF (CRLF is also accepted). Remove an entry to choose deletion.
