...
it cannot track both `data` and `data/model.bin`. The driver resolves same-path
conflicts but currently does not certify these constraints across merged paths
or shards; subsequent Gat reads reject an invalid combined lock.
