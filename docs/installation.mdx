...
<Steps>
  <Step title="Install gat">
    ...
    <Note>
      ...
      directory is missing from your <Tooltip tip="The list of directories your shell searches when you type a command such as gat.">PATH</Tooltip>
      or another `gat` command takes precedence. Follow the printed instructions
      to use the new installation by name, or run the full-path command provided
      by the installer. Opening a new shell alone does not add a directory to `PATH`.
    </Note>
  </Step>
...
<AccordionGroup>
  <Accordion title="Installer requirements and recovery">
    ...
    If temporary files cannot be removed, the installer prints their location
    for manual cleanup. This warning does not change the installation result.
  </Accordion>
  ...
  <Accordion
    title="Choose a Linux libc build"
    description="Use automatic selection or explicitly choose GNU or static musl."
  >
  ...
  >
    ...
    | --- | --- | --- |
    | Linux, glibc 2.28+ | x86_64 | `x86_64-unknown-linux-gnu` |
    | Linux, glibc 2.28+ | ARM64 | `aarch64-unknown-linux-gnu` |
    | Linux, static musl | x86_64 | `x86_64-unknown-linux-musl` |
    ...
    <Tabs>
      <Tab title="macOS / Linux">
        The example below shows Linux x86_64 with glibc 2.28+. For Alpine or
