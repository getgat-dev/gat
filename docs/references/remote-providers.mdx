  ...
  <Tab title="Azure" id="azure-blob-azblob">
    ...
    <Steps>
      <Step title="Choose credentials">
        ...
        | <Tooltip tip="A Shared Access Signature: a token granting specified storage permissions for a limited period. Supply it exactly as Azure provides it.">SAS token</Tooltip> | `AZURE_STORAGE_SAS_TOKEN` |
  ...
  <Tab title="GCS" id="google-cloud-storage-gcs">
    ...
    <Steps>
      <Step title="Set up Application Default Credentials">
        Set up <Tooltip tip="Google Cloud's credential discovery mechanism, used by applications to find a local credential file or workload identity.">Application Default Credentials</Tooltip> for local development:
...
delete operations you intend to use.
...
<CodeGroup>
  ...
  gat status --remote origin
  ```

  ```powershell Windows PowerShell
  $env:GAT_NETWORK_READINESS_TIMEOUT_SECONDS = '15'
  gat status --remote origin
  ```
</CodeGroup>
