...
| Command | Checks |
| --- | --- |
| `architecture` | Dependency direction, developer-tool isolation, environment acquisition/mutation, IO ownership, remote opening, root error rendering |
| `test-hygiene` | Environment mutation, unexplained sync/async sleeps, direct IO calls with shared paths or fixed/public endpoints |
...
shared paths or fixed endpoints are checked; loopback port zero is allowed.
...
`SHELL` chooses the Unix installer shell, not the fixture driver.
