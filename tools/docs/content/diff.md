Compare tracked paths and content IDs across <Tooltip tip="Git commit identifiers such as a branch, tag, commit hash, or HEAD~2. Gat reads the lock at each selected commit to compare file versions." cta="Resolve ambiguous names" href="/concepts/history-selection#when-a-branch-and-tag-share-a-name">revisions</Tooltip>.
This shows which file versions changed, not differences within binary contents.
When comparing the current lock, run {{command:add}} first to include unrecorded
file edits. Mount-owned entries show their mount name.
